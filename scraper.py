...
ANNOUNCEMENT_DETAIL_API = f"{BASE_URL}/api/pub/application-announcement-311/load-by-id"
LOGIN_URL = f"{BASE_URL}/auth/login"

# Конкурсы на услуги технического надзора — отдельная категория закупок
# (надзор за работами, а не сами работы), вне профиля парсинга. Сравниваем
# по корням слов, а не по точным фразам — так ловятся любые падежные формы
# ("технического надзора", "техническому надзору", "техническим надзором",
# "технический надзор" и т.п.), а не только буквально перечисленные
# варианты. "техническ\w*\s+надзор\w*" покрывает все русские падежи сразу;
...
def is_technical_supervision(text: str) -> bool:
    ...
    форме, рус./каз.). Применяется и к названию конкурса, и к полю
    "Наименование ЕНС ТРУ" из технической спецификации (см.
    get_enstru_name) — по отдельности они иногда расходятся: конкурс с
    непримечательным названием может внутри оказаться технадзором."""
    return bool(TECHNICAL_SUPERVISION_RE.search(text or ""))


def get_enstru_name(announcement_id):
    """
    Название по ЕНС ТРУ (справочник товаров/работ/услуг) конкурса — то же
    значение, что показано в карточке конкурса на вкладке "Техническая
    спецификация" в строке "Наименование ЕНС ТРУ". Название самого конкурса
    не всегда выдаёт технадзор — это поле выдаёт (обнаружено на живом
    примере: конкурс https://meks.zakup.sk.kz/board/announcement-view/
    ef8de3fe-9bcf-45f0-bcef-8e9732b31711 называется нейтрально, но его
    enstruName — "Услуги по осуществлению технического надзора…").
    Публичный эндпоинт, входа на портал не требует.

    Возвращает "" при любой ошибке запроса — тогда фильтр по этому полю
    просто не сработает для конкретного конкурса (это безопаснее, чем
    случайно посчитать сетевой сбой признаком технадзора и пропустить
...
def find_new_tenders(announcements, existing_tender_nos, status_cb=None):
    """
    ...
    Отбор в два шага, как смотрел бы человек:
    1) по названию самого конкурса (см. is_technical_supervision);
    2) если название ничего подозрительного не показало — донабираем по
       полю "Наименование ЕНС ТРУ" из технической спецификации карточки
       конкурса (см. get_enstru_name). Название конкурса иногда нейтрально,
       а это поле — прямо называет технадзор.
    Второй шаг делает по одному дополнительному запросу на каждый
    "новый по названию" конкурс, поэтому применяется только к кандидатам,
    прошедшим первый фильтр (а не ко всем announcements).

    status_cb: необязательная функция(str) для прогресса — второй шаг может
    занять заметное время, если новых конкурсов много.

    Возвращает (new_tenders, skipped_supervision) — второе число (по
    любому из двух признаков) для отображения в интерфейсе.
    """
